...
class PeriodicTask:
    ...
    def __init__(self, name, interval, base, exponent):
        ...
        self.thread = threading.Thread(target=self._run, daemon=True)
         # Добавляем флаг, чтобы знать, была ли задача запущена
    ...
    def _run(self):
        ...
        while not self.stop_event.is_set():
            ...
            for _ in range(int(self.interval * 10)):
                ...
                time.sleep(0.1)
            # Здесь задача выполняет свое полезное действие
            try:
    ...
    def stop(self):
        if self.is_started:
            self.stop_event.set()
            # Дожидаемся завершения потока, чтобы избежать ошибок при повторном запуске, если потребуется
...
class Scheduler:
    ...
    def __init__(self):
        self.tasks = {}
        # Добавляем новый параметр 'start_immediately'
...
def main():
   ...
   # Добавляем несколько задач при старте программы, но не запускаем их потоки
        ...
        if choice == '1':
            ...
            try:
                ...
                exponent_num = float(input("Введите степень: "))
                  # Новые задачи по умолчанию активны
            ...
            # При завершении программы, останавливаем только активные задачи
            ...
            print("Неверный ввод.")
# Вызываем main() напрямую, когда скрипт выполняется
main()